...
const ViewMail = (props) => {
  ...
  const deleteMailHandler = async () => {
    ...
    if (props.type === "recevied") {
      url = `https://mail-3c909-default-rtdb.firebaseio.com/rec${props.email}/${props.mail.id}.json`;
      dispatch(mailActions.deleteReceivedMail(props.mail.id));
    } else {
      url = `https://mail-3c909-default-rtdb.firebaseio.com/sent${props.email}/${props.mail.id}.json`;
